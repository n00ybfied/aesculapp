...
@Component({
  ...
  styles: [`
    ...
    .modal { width: min(100%, 28rem); border: 1px solid var(--admin-border); border-radius: 1rem; background: var(--admin-surface); padding: 1.5rem; color: var(--admin-text); box-shadow: 0 1.5rem 4rem rgb(0 0 0 / .22); }
    h2 { margin: 0; font-size: 1.25rem; }
    p { margin: .75rem 0 1.5rem; line-height: 1.5; }
    .actions { display: flex; justify-content: flex-end; gap: .75rem; }
    button { min-height: 2.75rem; border-radius: .6rem; padding: .6rem 1rem; font: inherit; font-weight: 700; cursor: pointer; }
    .cancel { border: 1px solid var(--admin-border); background: var(--admin-surface); color: var(--admin-text); }
    .confirm { border: 0; background: var(--admin-primary); color: white; }
    .confirm.destructive { background: var(--admin-danger); }
