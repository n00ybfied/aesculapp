...
export interface AdminRewardPage { rewards: readonly AdminReward[]; page: number; total: number; totalPages: number; }
...
export class AdminRewardService {
  ...
  async list(page = 1, pageSize: 10 | 25 | 50 | 'all' = 25, query = ''): Promise<AdminRewardPage> {
    return firstValueFrom(this.http.get<AdminRewardPage>(this.api + '/admin/rewards', {
      ...this.options(),
