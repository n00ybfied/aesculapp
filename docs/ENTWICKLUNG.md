...
Der separate Schlüssel wird einmalig mit `php bin/console app:chat:initialize-key` angelegt, lokal standardmäßig unter `server/var/private/chat.key`. Alternativ konfiguriert `CHAT_KEY_FILE` einen externen Pfad. Niemals einen verlorenen Schlüssel neu erzeugen, wenn verschlüsselte Nachrichten existieren: Ohne den ursprünglichen Schlüssel sind diese nicht wiederherstellbar. Schlüssel getrennt und sicher sichern, nicht in Git oder Deployment-Artefakte aufnehmen. Auf dem Demo-Server ist die Datei dem Deployment-Benutzer und der PHP-Gruppe lesbar (0640), der Ordner nicht öffentlich zugänglich (0750). Andere Anwendungen und Dienste werden nicht neu gestartet. Neue Installationen benötigen diesen Einrichtungsschritt vor Chat-Nutzung.
...
Die ausführbaren Symfony-Regressionstests liegen unter `server/tests/`. `php tests/receipt-integration.php` prüft den Loyalitäts-QR-Präfix, die Punkteberechnung aus dem punktefähigen Betrag, die unveränderliche Punktebuchung, den Dublettenschutz und den Debug-Modus für Wiederholungen. `php tests/chat-integration.php` prüft Zustimmung, Verschlüsselung, Zugriffsgrenzen, Idempotenz sowie Kunden- und Team-Badges für ungelesene Nachrichten. Beide Tests laufen innerhalb einer äußeren, am Ende zurückgerollten Datenbanktransaktion und hinterlassen keine Daten. `php tests/web-push.php` verwendet nur einen Mock-Transport und sendet keine echte Benachrichtigung; unter Laragon kann dafür `OPENSSL_CONF` auf die lokale OpenSSL-Konfiguration gesetzt werden.
